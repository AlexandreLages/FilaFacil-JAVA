...
<html>
  ...
  <body>
      ...
      <main class="mdl-layout__content mdl-color--grey-100">
		...
		<br>
		<form action="<c:url value="/ponto/associar"/>" method="post">
			<label>Ponto de Atendimento</label><br/>
			<select name="idPonto">
				<c:forEach var="ponto" items="${pontos}">
					<option id="${ponto}" value="${ponto.id}">${ponto.nome}</option>
				</c:forEach>
			</select><br/>
			<label>Unidade</label><br/>
			<select name="idUnidade">
				<c:forEach var="unidade" items="${unidades}">
					<option id="${unidade}" value="${unidade.id}">${unidade.nome}</option>
				</c:forEach>
			</select>
			<br/><br/><br/>
